...
contract Borrow is ReentrancyGuard{
    ...
    mapping (address => LiquidateInfo) public liquidateCollateralInfo;
    mapping (address => LiquidateInfo) public liquidateBorrowInfo;
    mapping( address => address ) public altexecutor;
    uint256 public borrowRate = 7500; // 75%    $100 collateral => $75 borrowable
    uint256 public liquidateRate = 8500; // 85%   
    uint256 public percentRate = 10000;
    uint8 public USDDECIMAL = 8;
    address public priceoracle = 0xe452d8b12684f0589b5768cf41A9a00144C96aD1;
    ...
    constructor(address _staking) {
        ...
        staking = _staking;
        executor = msg.sender;
        addBorrowAssets(0xbb4CdB9CBd36B01bD1cBaEBF2De08d9173bc095c,100,1e25);
        addBorrowAssets(0x55d398326f99059fF775485246999027B3197955,100,1e25);
        addBorrowAssets(0x8AC76a51cc950d9822D68b83fE1Ad97B32Cd580d,100,1e25);
        addBorrowAssets(0x0555E30da8f98308EdB960aa94C0Db47230d2B9c,100,1e25);
    }
    modifier onlyOperator() {
        require(operator == msg.sender, "SctGenesisPool: caller is not the operator");
        _;
    }
    ...
    function addBorrowAssets(
        ...
        borrowAssets.push(BorrowAssets({
            token : _token,
            apr : _apr,
            totalBorrowAmt : 0,
    ...
    function borrow( address user, address _btoken, uint256 _bamt ) public nonReentrant{
        require(msg.sender == user || msg.sender == altexecutor[user], "caller has no permission");
        require(IStaking(staking).getTotalValueUsable(_btoken) >= _bamt, "Total Usable Balance overflow");
        ...
        borrowInfo[user][_btoken].amountUSD += getAssetInUSD(_btoken, _bamt);
        borrowInfo[user][_btoken].lastBorrowTime = block.timestamp;
        IStaking(staking).transferToZynvous( _btoken, _bamt);
    ...
    function repay(address user, address _btoken, uint256 _bamt) public nonReentrant{
        require(msg.sender == user || msg.sender == altexecutor[user], "caller has no permission");
    ...
    function liquidate(address _user) public {
        require(getCollateralsInUSD(_user) * liquidateRate / percentRate <= getBorrowsInUSD(_user), "Not eligible to liquidate");

        UserInfo[] memory _userCollateralInfo = IStaking(staking).getUserInfo(_user);
        for( uint256 i = 0 ; i < _userCollateralInfo.length ; i++ ){
            address _ctoken = _userCollateralInfo[i].token;
            liquidateCollateralInfo[_ctoken].token = _ctoken;
            liquidateCollateralInfo[_ctoken].amount += _userCollateralInfo[i].borrowCollateralAmt;
        }
        ...
        for(uint256 i = 0; i < _borrowInfo.length; i++ ){
            ...
            liquidateBorrowInfo[_btoken].token = _btoken;
            liquidateBorrowInfo[_btoken].amount += _borrowInfo[i].amount;
        }
        IStaking(staking).borrowCollateralToBorrowedFunds(_user);
    }
    ...
    function checkPoolDuplicate(address _token) internal view {
        ...
        for (uint256 pid = 0; pid < length; ++pid) {
            require(borrowAssets[pid].token != _token, "StakingPool: existing pool?");
        }
    }
    ...
    function getBorrowsInUSD(address _user) public view returns(uint256){
        ...
        BorrowInfo[] memory _borrowInfo = getBorrowInfo(_user);
        for(uint256 i = 0; i < _borrowInfo.length; i++ ){
            _amtUSD += getAssetInUSD( _borrowInfo[i].token, _borrowInfo[i].amount );
        }
        return _amtUSD;
    }
    function getFeeAmount( address _user, address _btoken ) public view returns(uint256 feeAmount){
        uint256 lastborrowPeriod = block.timestamp - borrowInfo[_user][_btoken].lastBorrowTime;
        feeAmount = lastborrowPeriod * borrowInfo[_user][_btoken].amount * borrowAssets[tokenPid[_btoken]].apr / percentRate / rewardPeriod;
    }
    ...
    function getBorrowableAmt( address _user, address _btoken ) public view returns(uint256 _bamt) {
        uint256 _bamtUSD;
        if(getCollateralsInUSD(_user) * borrowRate / percentRate < getBorrowsInUSD(_user)){
            _bamtUSD = 0;
        }
        else{
            _bamtUSD = getCollateralsInUSD(_user) * borrowRate / percentRate - getBorrowsInUSD(_user);
        }
    ...
    function getCollateralRemovableAmt( address _user, address _ctoken ) public view returns(uint256) {
        ...
        if(getCollateralsInUSD(_user) <= getBorrowsInUSD(_user) / borrowRate * percentRate ){
            return _camt;
        }
        else{
            _camtUSD = getCollateralsInUSD(_user) - getBorrowsInUSD(_user) / borrowRate * percentRate;
        }
    ...
    function getCollateralRemoveAmtList( address _user ) public view returns( BorrowInfo[] memory ){
        UserInfo[] memory _userInfo = IStaking(staking).getUserInfo(_user);
    ...
    function getCollateralsInUSD(address _user) public view returns(uint256){
        ...
        UserInfo[] memory _userInfo = IStaking(staking).getUserInfo(_user);
        for( uint256 i = 0 ; i < _userInfo.length ; i++ ){
            _amtUSD += getAssetInUSD( _userInfo[i].token,_userInfo[i].borrowCollateralAmt);
        }
        return _amtUSD;
    }
    function getBorrowInfo(address _user) public view returns(BorrowInfo[] memory){
        BorrowInfo[] memory _borrowInfo = new BorrowInfo[](borrowAssets.length);
        for(uint256 i = 0; i < borrowAssets.length; i++){
            address _token = borrowAssets[i].token;
            uint256 _amount = borrowInfo[_user][_token].amount + getFeeAmount(_user, _token);
    ...
    function getBorrowableInfo(address _user) public view returns(BorrowableInfo[] memory){
        BorrowableInfo[] memory _borrowableInfo = new BorrowableInfo[](borrowAssets.length);
        for(uint256 i = 0; i < borrowAssets.length; i++){
            address _token = borrowAssets[i].token;
    ...
    function liquidateNeeded(address _user) public view returns(bool isNeeded){
        return getCollateralsInUSD(_user) * liquidateRate / percentRate < getBorrowsInUSD(_user);
    }
