...
interface IStaking {
    function transferToZynvous( address token, uint256 tokenAmount) external ;
...
contract Trade is ReentrancyGuard{
    ...
    address constant public priceoracle = 0xe452d8b12684f0589b5768cf41A9a00144C96aD1;
    ...
    uint256 constant public collateralLimit = 0; // 50 $50
    uint256 constant public tradeLimit_Down = 0; // 50 $50
    uint256 constant public tradeLimit_Up = 200000000000; // 200000 $200000
    ...
    // testing
    uint256 public TotalLeverageFee;
    uint256 public lossIncollateralToken_g;
    ...
    constructor(address _staking){
        ...
        setPriceFeed(0xbb4CdB9CBd36B01bD1cBaEBF2De08d9173bc095c,0x0567F2323251f0Aab15c8dFb1967E4e8A7D42aeE); // wbnb
        setPriceFeed(0x55d398326f99059fF775485246999027B3197955,0xB97Ad0E74fa7d920791E90258A6E2085088b4320); // usdt
        setPriceFeed(0x8AC76a51cc950d9822D68b83fE1Ad97B32Cd580d,0x51597f405303C4377E36123cBc172b13269EA163); // usdc
        setPriceFeed(0x0555E30da8f98308EdB960aa94C0Db47230d2B9c,0x264990fbd0A4796A3E3d8E37C4d5F87a3aCa5Ebf); // wbtc

        setUsingFeeApr(0xbb4CdB9CBd36B01bD1cBaEBF2De08d9173bc095c,3000); // wbnb
        setUsingFeeApr(0x55d398326f99059fF775485246999027B3197955,5000); // usdt
        setUsingFeeApr(0x8AC76a51cc950d9822D68b83fE1Ad97B32Cd580d,6000); // usdc
        setUsingFeeApr(0x0555E30da8f98308EdB960aa94C0Db47230d2B9c,2000); // wbtc

        addMarketPair(0xbb4CdB9CBd36B01bD1cBaEBF2De08d9173bc095c, 0x55d398326f99059fF775485246999027B3197955); // wbnb / usdt
        addMarketPair(0xbb4CdB9CBd36B01bD1cBaEBF2De08d9173bc095c, 0x8AC76a51cc950d9822D68b83fE1Ad97B32Cd580d); // wbnb / usdc
        addMarketPair(0x0555E30da8f98308EdB960aa94C0Db47230d2B9c, 0x55d398326f99059fF775485246999027B3197955); // wbtc / usdt
        addMarketPair(0x0555E30da8f98308EdB960aa94C0Db47230d2B9c, 0x8AC76a51cc950d9822D68b83fE1Ad97B32Cd580d); // wbtc / usdc
    ...
    function addMarketPair(address _long, address _short) public onlyOperator{
    ...
    function setUsingFeeApr( address _usingtoken, uint256 _apr ) public onlyOperator(){
        usingfeeapr[_usingtoken] = _apr;
    }
    function setPriceFeed(address _token, address _feed) public onlyOperator{
    ...
    function spotTradeWithCollateral(
        ...
        buyAmt = trade(swapDataParam, swapProxy_index, tokenIn, amountIn, tokenOut);
    ...
    function trade(
        ...
        IStaking(staking).transferToZynvous( tokenIn, amountIn);
        ...
        IERC20(tokenOut).transfer(treasury, _buyAmt - buyAmt);
    }
    ...
    function limitOrderExecute(
        ...
        for(uint256 i = 0 ; i < swapDataParam.length ; i++){
            require(limitorderinfo[_ids[i]].status, "This is canceld limitorder");
    ...
    function getLimitedOrderInfo(address _indexToken, uint256 _curPrice) public view returns(LimitOrderInfo[] memory){
        uint256[] memory _limitedIds = new uint256[](limitorderId);
        uint256 _index = 0;
        for( uint256 i = 1 ; i < limitorderId ; i++){
            if(!limitorderinfo[i].status || limitorderinfo[i].indexToken !=_indexToken){
                continue;
            }
            if(limitorderinfo[i].openPerform){
                if(limitorderinfo[i].highStart){ // start order when current price is higher than limit price
                    if(limitorderinfo[i].limitPrice < _curPrice){
                        _limitedIds[_index++] = i;
                    }
                }
                else{
                    if(limitorderinfo[i].limitPrice > _curPrice){
                ...
                if(limitorderinfo[i].upPrice > 0 && limitorderinfo[i].downPrice > 0){
                    if(limitorderinfo[i].upPrice < _curPrice){
                        _limitedIds[_index++] = i;
                    }
                    else if(limitorderinfo[i].downPrice > _curPrice){
                        _limitedIds[_index++] = i;
                    }
                }
                else if(limitorderinfo[i].upPrice > 0){
                    if(limitorderinfo[i].upPrice < _curPrice){
                        _limitedIds[_index++] = i;
                    }
                }
                else if (limitorderinfo[i].downPrice > 0){
                    if(limitorderinfo[i].downPrice > _curPrice){
                        _limitedIds[_index++] = i;
                    }
                }
                else{
        ...
        LimitOrderInfo[] memory _limitorderinfo = new LimitOrderInfo[](_index);
        for(uint256 i = 0 ; i < _index ; i++){
            _limitorderinfo[i] = limitorderinfo[_limitedIds[i]];
        }
        return _limitorderinfo;
    }
    
    function removeLimitOrderId(address _user, uint256 _limitorderid) private{
    ...
    function openPosition(
        ...
        require(_tradeInUSD <= tradeLimit_Up, "trade amount can't be more than up limit");
    ...
    function closeOpenId(
        ...
        // if there is no opend position of the user, then delete the user from the OpenedUsers Array.
    ...
    function closeAllUserOpenIds(
        ...
        uint256 _index;
        for( _index = 0 ; _index < len ; _index++){
            userCloseIds[_user].push(userOpenIds[_user][userOpenIds[_user].length-1]);
            userOpenIds[_user].pop();
        }
    }
    ...
    function closePosition(
        ...
        backAmt = trade(swapDataParam, swapProxy_index, _tokenOut, tradeinfo[_openId].amountOut, _tokenIn);
    ...
    function getLiquidateUsers() public view returns(address[] memory){
        ...
        for(uint256 i = 0 ; i < OpenedUsers.length ; i++){
            if(getTradableUSD(OpenedUsers[i]) == 0){
                _liquidateUsers[i] = OpenedUsers[i];
            }
        }
    ...
    function liquidatePosition(
        ...
        require(swapDataParam.length == swapProxy_index.length && userOpenIds[_user].length == swapProxy_index.length, "lengths are not matched");
        ...
        for(uint256 i = 0 ; i < userOpenIds[_user].length; i++){
            ...
            uint256 backAmt = trade(swapDataParam[i], swapProxy_index[i], _tokenOut, _amountOut, _tokenIn);
            tradeinfo[_openId].updateTime = block.timestamp;
            (, address _longtoken) = checkMarketPair(_tokenIn, _tokenOut);
            tradeinfo[_openId].closePrice = IPriceOracle(priceoracle).getPrice(_longtoken);
            tradeinfo[_openId].feeAmount = getFeeAmountById(_openId);
            ...
            if( backAmt >= _amountIn + leverageFee ){
                IStaking(staking).updateLockAmt(_tokenIn, backAmt - _amountIn - leverageFee, true); // increase the staking lock amount
            }
    ...
    function getTradableUSD(address _user) public view returns(uint256 _amountUSD){
        ...
        if(getCollateralsInUSD(_user) + getTradeOutTokensUSD(_user) > getTradeInTokensUSD(_user) + getFeeAmountUSDByUser(_user)){ 
            if((getCollateralsInUSD(_user) + getTradeOutTokensUSD(_user) - getTradeInTokensUSD(_user) - getFeeAmountUSDByUser(_user)) * MAXMULTIPLY > getOrderInTokenUSD(_user)){
    ...
    function getOrderInTokenUSD(address _user) public view returns(uint256){
        uint256 _amtUSD = 0;
        for(uint256 i = 0; i < userlimitOrderIds[_user].length; i++ ){
            uint256 _limitOrderId = userlimitOrderIds[_user][i];
    ...
    function getTradeOutTokensUSD(address _user) public view returns(uint256){
        uint256 _amtUSD = 0;
        TradeInfo[] memory _tradeInfo = getTradeInfo(_user);
        for(uint256 i = 0; i < _tradeInfo.length; i++ ){
